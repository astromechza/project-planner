...
export function TreeGrid({
  ...
  const [collapsedTaskIds, setCollapsedTaskIds] = useState<ReadonlySet<TaskId>>(
    () =>
      new Set(
  ...
  const handleKeyDown = (
    ...
    switch (event.key) {
      ...
      case 'ArrowRight':
        ...
        return;
      case 'ArrowLeft':
        if (task.childIds.length > 0 && !visibleCollapsedTaskIds.has(task.id)) {
          event.preventDefault();
          ...
          {visibleTasks.length === 0 ? (
            ...
            visibleTasks.map(({ task, depth }) => (
              <TreeRow
                ...
                blocksCount={
                  (dependencyCounts.get(task.id) ?? noDependencies).blocksCount
                }
                blockedByCount={
